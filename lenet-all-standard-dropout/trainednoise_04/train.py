...
def train():
	...
	with tf.Graph().as_default():
		...
		images = np.load('0.4.npy')
		images = images.reshape([55000,28,28,1])
		eval_data = mnist.test.images
		eval_data = eval_data.reshape([10000,28,28,1])

		x = tf.placeholder(tf.float32, shape=[None, 28,28,1]) #data gets loaded as a 28x8 vector
		...
		with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
			...
			for i in range(FLAGS.num_iter):
				...
				if i % 5000 == 0:
				    ...
				    validation_accuracy = accuracy.eval(feed_dict={x: eval_data, y: mnist.test.labels, keep_prob: 1.0}) 
...
if __name__ == '__main__':
    ...
    tf.app.flags.DEFINE_integer('num_iter', 250000, 'number of training iterations')
